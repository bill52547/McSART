__global__ void kernel_forwardDVF(float *mx, float *my, float *mz, float* alpha_x, float* alpha_y, float* alpha_z, float* beta_x, float* beta_y, float* beta_z, float* const_x, float* const_y, float* const_z, float volume, float flow, int nx, int ny, int nz)
{
    ...
    mx[id] = alpha_x[id] * volume + beta_x[id] * flow + const_x[id];
    my[id] = alpha_y[id] * volume + beta_y[id] * flow + const_y[id];
    mz[id] = alpha_z[id] * volume + beta_z[id] * flow + const_z[id];
