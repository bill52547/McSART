...
/*
        if (ibin == 0)
            ...
            cudaMemcpy(d_img1, h_img + (ibin - 1) * numImg, numBytesImg, cudaMemcpyHostToDevice);
        ...
        */
        cudaMemcpy(d_img, h_outimg + ibin * numImg, numBytesImg, cudaMemcpyHostToDevice);
        for (int i_view = n_views[ibin]; i_view < n_views[ibin + 1]; i_view++){
            ...
            h_outnorm[iter] = (float)sqrt(tempNorm * tempNorm + h_outnorm[iter] * h_outnorm[iter]);
